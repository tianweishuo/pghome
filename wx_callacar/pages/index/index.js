...
Page({
  ...
  callTaxi: function() {
    ...
    wx.request({
      ...
      success: function(res) {
        console.log(res.data.data.phoen);
        if (res.data.data.phoen != '') {
          //2.调用后台数据
          wx.request({
            url: app.globalData.url + '/passenger/call',
            method: 'post',
            data: {
              callPhone: _this.data.phone,
  ...
  sendmessag: function() {
    var _this = this;
    console.log("发送短信");
    //TODO 先使用正则进行手机号码位数验证
    if (_this.data.phone != null) {
      wx.request({
        url: app.globalData.url + '/wechat/registerPhone',
        data: {
          openId: _this.data.openid,
          ...
          if (res.data.code == 200) {
            _this.data.setInter = setInterval(function() {
              if (_this.data.timing == 0) {
                ...
                _this.setData({
                  timing: _this.data.timing - 1,
                  getcode: _this.data.timing,
                  isCode: true
                })
              }
            }, 1000);
  ...
  checkphoneclose: function() {
    var _this = this;
    _this.setData({
      phoneModelHidden: false
    })
  },
  checkphonesub: function() {
    console.log("提交");
  }
})